...
The repository uses a structured output layout for new runs. See
[`OUTPUT_LAYOUT.md`](OUTPUT_LAYOUT.md) for the `outputs/data`, `outputs/train`,
and `outputs/eval` naming and placement rules. Existing flat output directories
...
- **`LISA_ref_distill.py and LISA_sft_distill.py`**  
  Used in **Stage 1 (SFT training) and distillation**.
- **`LISA_grpo.py`**  
...
We also provide the checkpoint in [[baidu](https://pan.baidu.com/s/1xgPTW6VkkzOkywYbE2vmPg?pwd=jmwb),PIN:jmwb]. 
