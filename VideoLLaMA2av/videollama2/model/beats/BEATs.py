...
class BEATs(nn.Module):
    ...
    def extract_features(
        ...
        # Keep audio features on the module's device.  The released code
        # forced ``cuda:0``, which breaks ms-swift CPU probes and multi-GPU
        # placement (and can put inputs on a different rank than BEATs).
        device = self.patch_embedding.weight.device
        features = self.patch_embedding(fbank.to(device=device, dtype=self.patch_embedding.weight.dtype))
        # features = self.patch_embedding(fbank.half())
        # 在前向传播中
        # with torch.no_grad():
        #     cpu_fbank = fbank.cpu().half()
        #     cpu_features = self.patch_embedding.cpu()(cpu_fbank)
        #     features = cpu_features.to(fbank.device)  # 结果移回GPU
        T = features.shape[2]
        F = features.shape[3]
