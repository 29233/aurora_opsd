...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos(self, images):
        ...
        for i, (data, modal) in enumerate(images):
            if modal == 'image':
                ...
                data = data
            data_batch.append(data.bfloat16())

        vision_dtype = next(self.get_vision_tower().parameters()).dtype
        data_batch = torch.stack(data_batch, dim=0).to(device=self.device, dtype=vision_dtype)
        ...
        # frames_features = self.get_model().get_vision_tower().float().cpu()(frames.float().cpu())
        projector_dtype = next(self.get_model().mm_projector.parameters()).dtype
        frames_features = frames_features.to(device=data_batch.device, dtype=projector_dtype)
    ...
    def prepare_inputs_labels_for_multimodal1(
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, seg_idx=None
    ):
        vision_tower = self.get_vision_tower()
        audio_tower = self.get_audio_tower()
        # NOTE: text-only situation
        ...
        if audio_tower is None:
            ...
            if len(X_audio) > 0:
                Xa_features = torch.cat(X_audio, dim=0).to(self.device)
                audio_padding_mask = torch.zeros(Xa_features.shape, device=self.device).bool()
                audio_embedding, T, F = self.get_model().get_audio_tower().extract_features(Xa_features, padding_mask=audio_padding_mask, feature_only=True)
                Xa_features = self.get_model().mm_projector_a(audio_embedding)
                Xa_features = Xa_features.view(len(X_audio), -1, Xa_features.shape[-1])
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if cur_input_ids.numel() > 0:
                if seg_idx != None:
                    seg_token_mask = cur_input_ids == self.seg_token_idx
                    false_tensor = torch.full(((torch.cat(cur_new_input_embeds, dim=0)).shape[0],), False, dtype=torch.bool, device=labels.device)
                    seg_token_mask = torch.cat((false_tensor, seg_token_mask), dim=0)
                else:
                    seg_token_mask=None
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids))
                if labels is not None:
        ...
        return None, attention_mask, past_key_values, new_input_embeds, new_labels, seg_token_mask
