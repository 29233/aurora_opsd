...
class SiglipVisionTower(nn.Module):
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        # The released VideoLLaMA2 code used a machine-local absolute path.
        # Prefer the model config value and allow an explicit environment
        # override for offline deployments.
        vision_tower = os.environ.get("VIDEOLLAMA2_SIGLIP_PATH", vision_tower)
        self.vision_tower_name = vision_tower
        self.select_layer = args.mm_vision_select_layer
        self.select_feature = getattr(args, 'mm_vision_select_feature', 'patch')
    ...
    def load_model(self):
        ...
        try:
            # During ms-swift/Transformers low-memory loading this constructor
            # runs inside ``init_empty_weights``.  A nested from_pretrained
            # call would then try to move meta tensors to CPU and fail.
            self.vision_tower = SiglipVisionModel.from_pretrained(
                self.vision_tower_name, torch_dtype=torch.float16, device_map='cpu')
        except NotImplementedError as exc:
            if 'meta tensor' not in str(exc):
                raise
            vision_config = SiglipVisionConfig.from_pretrained(self.vision_tower_name)
